...
class SPOTT(SPOT):
    ...
    def forward(self, input, condit=None):
        ...
        - condit: condition bounding boxes, shape=(b,t,n,c), float32
        """
        ...
        if self.encode_backbone2:  # finetune self.encode_backbone
            with pt.inference_mode():
                feature2 = self.encode_backbone2(input).detach()  # (b*t,c,h,w)
            feature2 = feature2.clone()
        else:
            feature2 = feature.detach().clone()
        feature2 = feature2.unflatten(0, [b, t])
        ...
        if self.decode is None:
            return feature2, slotz, attent, None, None

        clue = rearrange(feature2, "b t c h w -> (b t) (h w) c")
